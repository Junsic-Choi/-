...
// .wrangler/tmp/bundle-Ia3FO8/checked-fetch.js
...
// .wrangler/tmp/bundle-Ia3FO8/middleware-loader.entry.ts
var import_checked_fetch40 = __toESM(require_checked_fetch());

// .wrangler/tmp/bundle-Ia3FO8/middleware-insertion-facade.js
var import_checked_fetch38 = __toESM(require_checked_fetch());

// .wrangler/tmp/pages-L7rcfL/functionsWorker-0.316650890865261.mjs
...
var require_checked_fetch2 = __commonJS2({
  "../.wrangler/tmp/bundle-miHfFY/checked-fetch.js"() {
    var urls = /* @__PURE__ */ new Set();
    function checkURL(request, init) {
      const url = request instanceof URL ? request : new URL(
        (typeof request === "string" ? new Request(request, init) : request).url
...
var import_checked_fetch2 = __toESM2(require_checked_fetch2(), 1);
  ...
  try {
    ...
    if (pastRows && pastRows.length > 0) {
      ...
      const carryoverData = latestWeekRows.map((row) => ({
        ...row,
        id: void 0,
        // Let the frontend/save layer handle new ids
        weekId,
        // Assign to the newly requested week
        mon: "",
        tue: "",
        wed: "",
        thu: "",
...
app.get("/plans-consolidated/:weekId", async (c) => {
  const weekId = c.req.param("weekId");
  try {
    const { results: allData } = await c.env.DB.prepare(
      `SELECT * FROM plans WHERE weekId <= ? ORDER BY weekId DESC, id ASC`
    ).bind(weekId).all();
    const consolidatedData = [];
    ALL_EQUIPMENTS.forEach((eq) => {
      const machineData = allData.filter((d) => d.equipment.trim() === eq.trim());
      if (machineData.length > 0) {
        const currentWeekRows = machineData.filter((d) => d.weekId === weekId);
        if (currentWeekRows.length > 0) {
          currentWeekRows.forEach((row) => consolidatedData.push(row));
        } else {
...
//# sourceMappingURL=functionsWorker-0.316650890865261.js.map
